...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = self.m[i](x[i])  # conv to: B,Ax(ClassNum+1+4),H,W    A为每个点的锚框个数
            bs, _, ny, nx = x[i].shape
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            # 形状变成了 [B,A,H,W,(ClassNum+1+4)] 如：[B,255,20,20] to [B,3,20,20,85]
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            if c2 != no:  # if not output
                c2 = make_divisible(c2 * gw, 8)  # 保证输出通道8的倍数，为什么？

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C3, C3TR, C3Ghost, C3x}:  # 这几个类型的积木的重复是在各积木的init里设置的，不是串联多个相同积木
        ...
        # TODO: channel, gw, gd
            ...
            c2 = ch[f]
        # 如果要串联多个积木，分别创建单个积木，再串联成Sequential；如果单个积木，直接创建这个积木
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module
        t = str(m)[8:-2].replace('__main__.', '')  # module type
        np = sum(x.numel() for x in m_.parameters())  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        LOGGER.info(f'{i:>3}{str(f):>18}{n_:>3}{np:10.0f}  {t:<40}{str(args):<30}')  # print
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist  这里对i取模什么意义？x应该不会大于i吧！
        layers.append(m_)
        if i == 0:
            ch = []
        ch.append(c2)
